...
<?php 
	if ( !isset($_GET['adminView']) AND $app_form->type != ApplicationForm::TYPE_PAID )
	{
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<div>
				...
				{
					if($app_form->type == ApplicationForm::TYPE_PAID AND !empty($app_form->promo_code)) 
					{
						$statusText = 'Free Promo Code';
					}
					else
					{
						$statusText = $app_form->getTypeLabel($app_form->type); 
					}
					
					echo '<strong>Credit Application Status: '.$statusText.'</strong>';
				}
				else
				{
				?>
					<strong>(A $<?php echo PaymentAPI::PAYMENT_CREDIT_APPLICATION_PRICE; ?> non-refundable application processing fee will be charged)</strong>
				<?php
				}
				?>
			</div> <br />
					...
					<label class=" checkbox-inline">
                        ...
                        <div class="row row-padding">
                            ...
                            <div class="col-xs-8" style="word-wrap: break-word;">
                                <div class="form-style">
                                    <?php echo $app_form->exempt_certificate_path; ?>
                                </div>
                            </div>
                        </div>
                        <?php endif; ?>
			...
			<div class="form-horizontal">
				...
				<div class="form-group">
					...
					<div class="col-xs-2">
						<div class="radio">
							<label>
								<input value="12" name="VendorInfo[term]" id="VendorInfo_term" disabled="disabled" <?php echo ($vendor->term == 12)? 'checked="checked"': "";  ?> type="radio">								12mo
							</label>
						</div>
					</div>
					<div class="col-xs-2">
						<div class="radio">
							<label>
								<input value="24" name="VendorInfo[term]" id="VendorInfo_term" disabled="disabled" <?php echo ($vendor->term == 24)? 'checked="checked"': "";  ?> type="radio">								24mo
							</label>
						</div>
					</div>
					<div class="col-xs-2">
						<div class="radio">
							<label>
								<input value="36" name="VendorInfo[term]" id="VendorInfo_term" disabled="disabled" <?php echo ($vendor->term == 36)? 'checked="checked"': "";  ?> type="radio">								36mo
							</label>
						</div>
					</div>
					<div class="col-xs-2">
						<div class="radio">
							<label>
								<input value="46" name="VendorInfo[term]" id="VendorInfo_term" disabled="disabled" <?php echo ($vendor->term == 46)? 'checked="checked"': "";  ?> type="radio">								46mo
							</label>
						</div>
					</div>
					<div class="col-xs-2 col-xs-offset-4">
						<div class="radio">
							<label>
								<input value="60" name="VendorInfo[term]" id="VendorInfo_term" disabled="disabled" <?php echo ($vendor->term == 60)? 'checked="checked"': "";  ?> type="radio">								60mo
							</label>
						</div>
					</div> 
				</div>
		...
		<div class="col-xs-6">
			<div class="form-horizontal">
				...
				<div class="form-group" style="margin-bottom:8px;">
					<label class="col-xs-4 control-label"><?php 
				...
				<div class="form-group">
					...
					<div class="col-xs-4 phone">
						<div class="radio">
							<label>
                                                                <input value="1" name="VendorInfo[new_flag]" id="VendorInfo_new_flag"  <?php echo ((int)$vendor->new_flag == 1)? 'checked="checked"': "";  ?> type="radio" disabled="disabled">	New
							</label>
						</div>
					</div>
					<div class="col-xs-4 phone">
						<div class="radio">
							<label>
								<input value="0" name="VendorInfo[new_flag]" id="VendorInfo_new_flag"  <?php echo ((int)$vendor->new_flag == 0)? 'checked="checked"': "";  ?> type="radio" disabled="disabled"> Used
							</label>
						</div>
					</div>
				</div>
...
<div>
    <?php if ( ApplicationForm::TYPE_PAID != $app_form->type ) {
			if ( !isset($_GET['adminView']) ) { ?>
    <div class="row">
		<div class="col-sm-6 text-right">
			<br />
			<i>*Scroll up to view your application form</i>
		</div>
        <div class="col-sm-6">
            <?php echo CHtml::link('Checkout', array('/checkout/setApplicationForm', 'id' => $app_form->id, 'page' => 'checkout'), array('class' => 'btn btn-primary orange-btn pull-right', 'style' => 'margin-left: 10px;', 'id' => 'checkoutButton')); ?>
            <?php 
			echo CHtml::link('Update Credit Application', array('/applicationForm/creditApplication', 'id' => $app_form->id), array('class' => 'btn btn-primary pull-right', 'style' => 'padding: 5px 15px; font-size: 24px; font-family: \'bebas_neue_regularregular\',Arial;')); 
			Yii::app()->clientScript->registerScript('checkoutButtonScript', "
				$('html, body').animate({
