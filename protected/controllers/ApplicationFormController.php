...
class ApplicationFormController extends Controller
{
	...
	public function actionCreditApplication($id = null, $startNew = null)
	{
		...
		if (empty($_SERVER["HTTPS"]) || $_SERVER["HTTPS"] !== "on")		//force to https
		{
			...
			switch ($step) {
				...
				case '2': case 2:
					...
					if(isset($_POST['PrincipalInfo'])) {
						...
						if(!empty($data1['name_title']) OR !empty($data1['address']) OR !empty($data1['location']) OR !empty($data1['ownership']) OR !empty($data1['security_number']) OR !empty($data1['email_address'])) {
						...
						if(!empty($data2['name_title']) OR !empty($data2['address']) OR !empty($data2['location']) OR !empty($data2['ownership']) OR !empty($data2['security_number']) OR !empty($data2['email_address'])) {
						...
						if(!empty($data3['name_title']) OR !empty($data3['address']) OR !empty($data3['location']) OR !empty($data3['ownership']) OR !empty($data3['security_number']) OR !empty($data3['email_address'])) {
					...
					if(isset($_POST['VendorInfo'])) {
						$vendor->setAttributes($_POST['VendorInfo']);
							
						$validate = 0;
						
						$vendor->other_line_of_credit = $_POST['other'];
		...
		if(isset($_POST['ApplicationForm']) AND !empty($_POST['applicationCreditFormProcess']))
		{
			...
			$app_form -> attributes = $_POST['ApplicationForm'];
				if(isset($_POST['validated_promo_code'])) {
					...
					$app_form->promo_code = $validatedPromoCode;
					$_SESSION['ap_frm_promo_code'] = $validatedPromoCode;
				}
					...
					if($valid)
					{
						...
						if($valid)
						{
							...
							if ( isset($_POST['applicationCreditFormProcess']) )
							{
								...
								{
									if ( Yii::app()->user->isGuest )
										$redirectUrl = array('site/register','sendTo' => 'checkout');
									else
	...
	public function actionDynamicCity_backup() 
	{
	...
	public function actionDynamicCity() 
	{
		...
		$data=Cities::model()->findAll('state_code=:s_code',
				array(':s_code'=> $_POST['state']));


		$data=CHtml::listData($data,'id','city');
		$option = array();
		foreach($data as $value=>$subcategory)  {
			$option[] = CHtml::tag('option',
			   array('value'=>$value),CHtml::encode($subcategory),true);
		}
	...
	public function actionCreditApplicationDev($id = null, $startNew = null)
	{
		// echo 'in'; exit;
		// echo '<pre>';
		// print_r($state); exit;
					...
					if($valid)
					{
						...
						if($valid)
						{
							...
							if ( isset($_POST['applicationCreditFormProcess']) )
							{
								...
								{
									if ( Yii::app()->user->isGuest )
										$redirectUrl = array('site/login','sendTo' => 'checkoutDev');
									else
										$redirectUrl = array('checkout/dev', 'id' => $app_form->id);
								}
								else
								{
									$redirectUrl = array('site/login','sendTo' => 'saveApplicationForm');
        ...
        {
            $authAccount = Yii::app()->user->account;
            $applicationForm = ApplicationForm::model()->find(array(
                'condition' => "id = :id ", //AND account_id = :accountId
                'params' => array(
                    ':id' => $id,
			...
			// if( Yii::app()->user->account->id == 47 )
				// echo $urlPath;exit;
			
			if( PaypalAPI::getInstance()->apiLive == false )
				$subjectTitle = '(TEST ONLY)'.$subjectTitle;
			
            $filename = $subjectTitle.'.pdf';
			
			// if( Yii::app()->user->account->id == 47 )
			// {
				// $filename = 'testrsa.pdf';
			// }
            $pdfPath = Yii::getPathOfAlias('webroot').'/file-uploads/'.$filename;

			// if( Yii::app()->user->account->id == 47 )
			// {
				// Dev::pvx("wkhtmltopdf -B '5mm' -L '5mm' -R '5mm' -T '5mm' '".$urlPath."' '".$pdfPath."'");
			// }
