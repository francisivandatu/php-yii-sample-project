...
class PaypalApiController extends Controller
{
		...
		if ( $success == 'true' )
		{
			...
			try
			{
				...
				if ( $payment['status'] == 'success' )
				{
					...
					//payment was successfull, update the trasaction details
					$paymentTransaction->txn_id = $payment->transactions[0]->invoice_number; // invoice_number of transaction paypal
					...
					/* EMAIL NOTIFICATION */
					$res = $this->sendMails($applicationForm, $account->userBillingInfo, $payment, $paymentTransaction);
	...
	private function sendMails($applicationForm, $userBillingInfo, $paymentAPI, $paymentTransaction)
	{
		...
		//generate admin email
		{
			...
			if ( PaypalAPI::getInstance()->apiLive == true )
			{
				$testFlag = '';
				$emailBcc = array(
